namespace :cron do
  ...
  task :upload do
    ...
    RequestsSource.all.each do |source|
      source.download_urls.each do |url|
        value = client.get(url).body
        shasum = Digest::SHA1.hexdigest(value)
        shasum_filename = "#{source.jurisdiction_code}.shasum"
        unless store.exist?(shasum_filename) && store.read(shasum_filename) == shasum
          data_filename = url_to_basename(url)
          store.write(File.join(source.jurisdiction_code, now.strftime('%Y'), now.strftime('%Y-%m-%d'), data_filename), value)
